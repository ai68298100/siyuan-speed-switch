...
test('reconcileStorageCapacityReport repairs summary flags and trend action', () => {
    const repaired = reconcileStorageCapacityReport({version: 1, health: {near: ['favorites']}, trend: {trend: 'stable', action: 'none'}, summary: {changedBuckets: ['favorites'], changed: false, changedCount: 0}});
    assert.equal(repaired.summary.changed, true);
    assert.equal(repaired.summary.changedCount, 1);
    assert.equal(repaired.trend.action, 'monitor');
});

test('reconcileStorageCapacityReport returns fixed safe report for malformed input', () => {
    const repaired = reconcileStorageCapacityReport(null);
    assert.equal(repaired.version, 1);
    assert.deepEqual(Object.keys(repaired), ['version', 'health', 'trend', 'summary']);
});

test('buildStorageCapacityReportEvents emits bounded stable event types', () => {
    const events = buildStorageCapacityReportEvents({health: {used: 1, max: 10}}, {health: {used: 9, max: 10, near: ['favorites']}});
    assert.equal(events.some((event) => event.type === 'risk_changed'), true);
    assert.equal(events.some((event) => event.type === 'usage_trend'), true);
});
...
test('createStorageCapacityReportEventQueue bounds, reads, and acknowledges events', () => {
    const queue = createStorageCapacityReportEventQueue(2);
    assert.equal(queue.enqueue([{type: 'usage_trend', direction: 'up'}, {type: 'usage_trend', direction: 'down'}]).accepted, 2);
    queue.enqueue([{type: 'near_capacity', buckets: ['favorites']}]);
    assert.equal(queue.snapshot().size, 2);
    assert.equal(queue.read(0).truncated, true);
    assert.equal(queue.read(1).events.length, 2);
    assert.equal(queue.acknowledge(2).removed, 1);
});

test('createStorageCapacityReportEventQueue isolates dispose and malformed enqueue', () => {
    const queue = createStorageCapacityReportEventQueue();
    assert.equal(queue.enqueue([{type: 'bad'}]).accepted, 0);
    queue.dispose();
    assert.equal(queue.snapshot().disposed, true);
    assert.equal(queue.read().disposed, true);
});

test('replayStorageCapacityReportEvents reads bounded events and optionally acknowledges', () => {
    const queue = createStorageCapacityReportEventQueue(4);
    queue.enqueue([{type: 'usage_trend', direction: 'up'}]);
    const replay = replayStorageCapacityReportEvents(queue, {acknowledge: true, limit: 1});
    assert.equal(replay.mode, 'events');
    assert.equal(replay.acknowledged, 1);
    assert.equal(queue.snapshot().size, 0);
});

test('replayStorageCapacityReportEvents preserves queue on cancel, timeout, and unavailable queue', () => {
    const queue = createStorageCapacityReportEventQueue();
    queue.enqueue([{type: 'usage_trend', direction: 'up'}]);
    assert.equal(replayStorageCapacityReportEvents(queue, {signal: {aborted: true}}).mode, 'cancelled');
    assert.equal(replayStorageCapacityReportEvents(queue, {deadline: Date.now() - 1}).mode, 'timeout');
    assert.equal(replayStorageCapacityReportEvents(null).mode, 'failed');
    assert.equal(queue.snapshot().size, 1);
});

test('recoverStorageCapacityReportEventQueue returns events or snapshot fallback', () => {
    ...
    assert.equal(recoverStorageCapacityReportEventQueue(queue).mode, 'snapshot_required');
    const recovered = recoverStorageCapacityReportEventQueue(queue, {favorites: {used: 1, max: 10}});
    assert.equal(recovered.mode, 'snapshot');
    assert.equal(recovered.snapshot.version, 1);
});

test('recoverStorageCapacityReportEventQueue isolates disposed queues', () => {
    const queue = createStorageCapacityReportEventQueue();
    queue.dispose();
    assert.equal(recoverStorageCapacityReportEventQueue(queue).reason, 'queue_disposed');
});

test('createStorageCapacityReportEventCoordinator serializes commits and rejects stale cursors', () => {
    const queue = createStorageCapacityReportEventQueue(4);
    queue.enqueue([{type: 'usage_trend', direction: 'up'}]);
    const coordinator = createStorageCapacityReportEventCoordinator(queue);
    const result = coordinator.recover();
    assert.equal(result.cursor, 1);
    assert.equal(coordinator.commit(1).committed, true);
    assert.equal(coordinator.commit(0).reason, 'stale_cursor');
});

test('createStorageCapacityReportEventCoordinator isolates disposal', () => {
    const coordinator = createStorageCapacityReportEventCoordinator(createStorageCapacityReportEventQueue());
    coordinator.dispose();
...
test('queue signal/deadline readers preserve cancellation and timeout semantics', () => {
    const queue = createStorageCapacityReportEventQueue();
    queue.enqueue([{type: 'usage_trend', direction: 'up'}]);
...
test('custom icon normalization accepts bounded HTTPS/data images and rejects credentials/oversize data', () => {
    const data = 'data:image/png;base64,' + 'A'.repeat(32);
    assert.equal(normalizeCustomIcon(data), data);
    assert.deepEqual(resolveIconReference(data, new Set(['iconFile'])), {type: 'image', value: data});
    assert.equal(isImageIconReference(data), true);
    assert.equal(normalizeCustomIcon('https://cdn.example.com/icon.png'), 'https://cdn.example.com/icon.png');
    assert.deepEqual(resolveIconReference('https://cdn.example.com/icon.png', new Set(['iconFile'])), {
        type: 'image', value: 'https://cdn.example.com/icon.png',
    });
    assert.equal(normalizeCustomIcon('https://user:pass@example.com/icon.png'), undefined);
    assert.equal(normalizeCustomIcon('data:image/png;base64,' + 'A'.repeat(240001)), undefined);
});
