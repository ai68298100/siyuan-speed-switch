...
test("document-context groundwork keeps a bounded read-only contract", () => {
    ...
    assert.deepEqual(Object.fromEntries(Object.entries(context).filter(([key]) => !["source", "outlineAvailable", "outlineStatus", "metadataStatus", "pathSource", "notebookName", "pathAvailable"].includes(key))), {
