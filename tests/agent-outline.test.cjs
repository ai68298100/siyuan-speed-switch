...
test("document-context groundwork keeps a bounded read-only contract", () => {
    ...
    assert.deepEqual(Object.fromEntries(Object.entries(context).filter(([key]) => !["source", "outlineAvailable", "outlineStatus", "metadataStatus", "metadataMissing", "pathSource", "pathReason", "notebookName", "pathAvailable"].includes(key))), {
