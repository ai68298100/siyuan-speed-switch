...
test('package.zip, when present, contains only release files', (t) => {
    ...
    const allowed = /^(index\.js|index\.css|icon\.png|preview\.png|README(?:\.en-US)?\.md|plugin\.json|i18n\/(?:en|zh-CN)\.json|docs\/(?:architecture|interface-map)\.svg|docs\/component-store-guide\.md)$/;
