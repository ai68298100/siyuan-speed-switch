...
test('dependency list defaults to catalog', () => assert.equal(model.listHomeStoreDependencies().length, 14));
...
test('dependency summary counts required and optional entries', () => { const summary = model.summarizeHomeStoreDependencies(); assert.equal(summary.total, 14); assert.equal(summary.required, 8); assert.equal(summary.optional, 6); });
