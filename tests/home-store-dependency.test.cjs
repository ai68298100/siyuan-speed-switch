...
test('dependency list defaults to catalog', () => assert.equal(model.listHomeStoreDependencies().length, 11));
...
test('dependency summary counts required and optional entries', () => { const summary = model.summarizeHomeStoreDependencies(); assert.equal(summary.total, 11); assert.equal(summary.required, 7); assert.equal(summary.optional, 4); });
test('dependency summary exposes kind histogram', () => { const summary = model.summarizeHomeStoreDependencies(); assert.equal(summary.kinds['external-api'], 4); assert.equal(summary.kinds['self-hosted-api'], 3); });
