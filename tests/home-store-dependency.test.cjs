...
test('dependency catalog includes all non-core providers', () => assert.deepEqual(Object.keys(model.DEPENDENCY_INFO).sort(), ['checkin-monthly','checkin-occasions','checkin-streak','checkin-summary','checkin-today','checkin-weekly','checkin-year-heatmap','external-activitywatch-time','external-air-quality','external-anime-bangumi','external-fx-frankfurter','external-github-contrib','external-hot-news-dailyhot','external-ical-events','external-news-hackernews','external-news-newsnow','external-rss-miniflux','external-rss-subscription','external-status-uptimekuma','external-weather-open-meteo','journal-calendar','plugin-commands']));
...
test('dependency list defaults to catalog', () => assert.equal(model.listHomeStoreDependencies().length, 22));
...
test('dependency summary counts required and optional entries', () => { const summary = model.summarizeHomeStoreDependencies(); assert.equal(summary.total, 22); assert.equal(summary.required, 15); assert.equal(summary.optional, 7); });
test('dependency summary exposes kind histogram', () => { const summary = model.summarizeHomeStoreDependencies(); assert.equal(summary.kinds['external-api'], 6); assert.equal(summary.kinds['self-hosted-api'], 4); });
