...
test("ADR 0067 recalibrates the raw bundle self-discipline line for B4", () => {
    ...
    assert.equal(releaseMetrics.RAW_BUNDLE_BUDGET_BYTES, 896 * 1024);
    ...
    assert.match(gateRaw, /2026-09-21 \(ADR 0067\)/);
    const adr = fs.readFileSync(path.join(__dirname, "..", "docs", "adr", "0067-raw-bundle-line-recalibration.md"), "utf8");
    assert.match(adr, /896 KiB/);
