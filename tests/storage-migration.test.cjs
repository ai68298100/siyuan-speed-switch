...
test('key classification cannot drift: totals stay 13 with no duplicate or unfiled key', () => {
    // 总数是**字面量**断言：report.keys.length === KEY_ORDER.length 是自指恒真式，
    // 无法发现「同一 key 同时出现在两个分类集里」导致的重复。13 与
    // agent-capabilities 的只读快照上限同源（buildAgentStorageHealth 钳制到 13）。
    assert.equal(KEY_ORDER.length, 13, 'storage key total is the contract other modules clamp against');
    assert.equal(new Set(KEY_ORDER).size, KEY_ORDER.length, 'KEY_ORDER must not contain duplicates');
    assert.equal(KEY_ORDER.length, HANDLED_KEYS.length + INSPECTED_KEYS.length,
        'every key is filed in exactly one classification set');
    assert.equal(HANDLED_KEYS.length + INSPECTED_KEYS.length, new Set([...HANDLED_KEYS, ...INSPECTED_KEYS]).size,
        'no key may be filed in both sets (would silently report the same key twice)');
    for (const key of [...HANDLED_KEYS, ...INSPECTED_KEYS]) {
