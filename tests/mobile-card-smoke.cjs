...
const documentSetContractOk = source.includes('"documentSets"')
    ...
    && sectionsSource.includes('restoreController.abort()')
    && sectionsSource.includes('documentSetRestoreCancelled')
    && sectionsSource.includes('documentSetRestoreSkipped')
    && sectionsSource.includes('documentSetRestoreMissing')
    && sectionsSource.includes('!restore.isConnected')
    && sectionsSource.includes('this.isUnloading || !restore.isConnected')
    ...
    && sectionsSource.includes('file.size > DOCUMENT_SET_IMPORT_MAX_BYTES')
    && sectionsSource.includes('documentSetImportFailed')
    && sectionsSource.includes('documentSetImportConfirm')
    && source.includes('documentSetUnknownConfirm')
    && sectionsSource.includes('importButton.setAttribute("aria-busy", "true")')
    && sectionsSource.includes('importButton.removeAttribute("aria-busy")')
    && sectionsSource.includes('siyuan-speed-switch-document-sets.json')
    && sectionsSource.includes('editingSetId')
