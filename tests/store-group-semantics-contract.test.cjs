...
test('group metadata type includes a description field', () => {
    assert.match(source, /Array<\{label: string; description: string; moduleIds: string\[\]\}>/);
});
test('group metadata keeps module ids explicit', () => assert.match(source, /moduleIds: \[/));
test('group order starts with journal', () => assert.match(source, /BUILTIN_GROUPS:[\s\S]*?homeStoreGroupJournal/));
test('group order places tasks after journal', () => assert.ok(source.indexOf('homeStoreGroupJournal') < source.indexOf('homeStoreGroupTasks')));
test('group order places documents after tasks', () => assert.ok(source.indexOf('homeStoreGroupTasks') < source.indexOf('homeStoreGroupDocuments')));
test('group order places insights after documents', () => assert.ok(source.indexOf('homeStoreGroupDocuments') < source.indexOf('homeStoreGroupInsights')));
test('group order places learning after insights', () => assert.ok(source.indexOf('homeStoreGroupInsights') < source.indexOf('homeStoreGroupLearning')));
test('group order places life after learning', () => assert.ok(source.indexOf('homeStoreGroupLearning') < source.indexOf('homeStoreGroupLife')));
test('group order places system last', () => assert.ok(source.indexOf('homeStoreGroupLife') < source.indexOf('homeStoreGroupSystem')));
test('journal hint is wired to journal group', () => assert.match(source, /homeStoreGroupJournal, description: this\.i18n\.homeStoreGroupJournalHint/));
test('tasks hint is wired to tasks group', () => assert.match(source, /homeStoreGroupTasks, description: this\.i18n\.homeStoreGroupTasksHint/));
test('documents hint is wired to documents group', () => assert.match(source, /homeStoreGroupDocuments, description: this\.i18n\.homeStoreGroupDocumentsHint/));
test('insights hint is wired to insights group', () => assert.match(source, /homeStoreGroupInsights, description: this\.i18n\.homeStoreGroupInsightsHint/));
test('learning hint is wired to learning group', () => assert.match(source, /homeStoreGroupLearning, description: this\.i18n\.homeStoreGroupLearningHint/));
test('life hint is wired to life group', () => assert.match(source, /homeStoreGroupLife, description: this\.i18n\.homeStoreGroupLifeHint/));
test('system hint is wired to system group', () => assert.match(source, /homeStoreGroupSystem, description: this\.i18n\.homeStoreGroupSystemHint/));
...
test('journal module list contains writing streak', () => assert.match(source, /"writing-streak"/));
test('task group contains today tasks', () => assert.match(source, /"today-tasks", "countdown"/));
test('task group contains quick capture', () => assert.match(source, /"quick-capture", "clipped-unread"/));
...
test('description map is separate from card grouping', () => assert.match(source, /const readyGroupDescriptions = new Map<string, string>\(\)/));
test('description map is populated once per group', () => assert.match(source, /if \(!readyGroupDescriptions\.has\(label\)\) readyGroupDescriptions\.set/));
test('description is rendered as text', () => assert.match(source, /groupDescription\.textContent = readyGroupDescriptions\.get\(label\)/));
test('description is linked with aria-describedby', () => assert.match(source, /groupHeading\.setAttribute\("aria-describedby", descriptionId\)/));
test('description uses a dedicated class', () => assert.match(source, /groupDescription\.className = "sw-home-store__group-description"/));
test('description id is deterministic', () => assert.match(source, /sw-home-store-group-description-\$\{orderedGroups\.indexOf\(label\)\}/));
