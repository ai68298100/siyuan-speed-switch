...
//   （局限：同选择器在文件里有**多条**规则时，删除式探针只达**首条**承载该声明的规则——
//   实测 `card:hover { transform: none }` 在 560px 与 reduce 两个分支各一条，探针删的是
//   前者、翻不动钉在 reduce 分支的断言。此类断言用 md5 保护的手工外科注入补验，并把
//   局限记录在案；不要据此断言"不守护任何东西"。）
