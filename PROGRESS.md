...
验证基线：T-066~T-102 后 `pnpm verify:release` 全绿，包含 TypeScript、生产构建、571/571、移动烟测和 Chromium 样式烟测；raw bundle 自律预算 345 KiB，压缩包约 298.7 KiB，条目级差异与硬上限余量均由 diagnostic 持续观测。

待处理：

1. 继续保持完整自动门禁和 300 KiB 压缩包硬上限。
2. 下一候选：先完成 T-104 包体瘦身，再推进 T-103 桌面路径筛选原型；真实窄屏验收按 D-042 后置。
