...
当前基线：`v0.16.39`（发布提交 `3d0d841`），开发头基于 `72e2deb`；本地 `main` 已包含未推送的性能/UI提交，T-071~T-074 已完成。正式发版继续后置。
...
验证基线：T-066~T-074 后 `pnpm verify:release` 全绿，包含 TypeScript、生产构建、556/556、移动烟测和 Chromium 样式烟测；raw bundle 自律预算 342 KiB，压缩包继续低于 300 KiB 硬上限。

待处理：

1. T-075：性能可观测性摘要（只读、默认关闭、严格采样上限）。
