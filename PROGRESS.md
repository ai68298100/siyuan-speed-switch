...
当前基线：`v0.16.39`（发布提交 `3d0d841`），开发头为本地 `main`；本地分支已包含未推送的性能/UI提交，T-071~T-083 已完成。正式发版继续后置。
...
验证基线：T-066~T-087 后 `pnpm verify:release` 全绿，包含 TypeScript、生产构建、565/565、移动烟测和 Chromium 样式烟测；raw bundle 自律预算 345 KiB，压缩包继续低于 300 KiB 硬上限。

待处理：

1. 继续保持完整自动门禁和 300 KiB 压缩包硬上限。
2. 下一候选：刷新按钮 busy/disabled 的低动效视觉反馈；首页状态文案布局回归继续等待真实溢出证据。
