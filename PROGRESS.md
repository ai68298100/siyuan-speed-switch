...
当前基线：`v0.16.39`（发布提交 `3d0d841`），开发头为本地 `main`；本地分支已包含未推送的性能/UI提交，T-071~T-114 已完成。正式发版继续后置。
...
验证基线：T-066~T-120 后 `pnpm verify:release` 全绿，包含 TypeScript、生产构建、579/579、移动烟测和 Chromium 样式烟测；raw bundle 自律预算 345 KiB，压缩包约 294.6 KiB，条目级差异与硬上限余量均由 diagnostic 持续观测。
