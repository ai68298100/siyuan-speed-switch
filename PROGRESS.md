...
本轮新增（2026-09-13）：T-150 为 workspace bridge 增加显式过期计划回收 `prune(now)`，T-151 增加 capability 注册句柄卸载回收，均用于长期运行时和插件销毁阶段清理陈旧状态；606/606 自动测试、TypeScript 与 diff 检查通过。生产 bundle 暂不接入该独立模块。
