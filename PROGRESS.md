...
当前基线：`v0.16.39`（发布提交 `3d0d841`），开发头 `72e2deb`。本地 `main` 与 GitHub `origin/main` 已同步；正式发版继续后置。
...
验证基线：T-066~T-069 后 `pnpm verify:release` 全绿，包含 TypeScript、生产构建、553/553、移动烟测和 Chromium 样式烟测；`index.js` 346932 字节，`package.zip` 306232 字节，raw bundle 自律预算 339 KiB。

待处理：

1. T-070~T-071：从卡片配色、农历辅助信息中选择下一组不依赖真机的任务。
2. 继续保持完整自动门禁和 300 KiB 压缩包硬上限。
...
本轮新增（2026-09-13）：journal-calendar 日历月视图（第 27 个内置）与 writing-streak（第 28 个内置，协议 v2.3 `viewType=weekdays`）；完成 `stat.arc` 进度环渲染器，年度进度与写作打卡均提供 SVG progressbar；D-042 已录（跳过常规设备测试）。后续候选：卡片配色预设、农历辅助、面板首开延迟首读。
UI 打磨（本轮）：折叠按钮文字→箭头图标；空状态淡色前缀；日历格悬停反馈；条目悬停左侧强调色条；stat hero 数字加大加粗；面板内嵌套外壳已去除（上轮）。本地 31 commit 待推送。
性能+交互优化（本轮）：组件面板打开时前两项立即读取、其余按空闲回调/80ms 阶梯延迟，且销毁时清理尾部任务；折叠体 max-height+opacity 平滑动画；进度条圆角端点；条目悬停过渡；新增进度环按需渲染。后续候选：月历翻月按钮、卡片配色预设、农历支持、缩略图缓存按需降载。
