...
验证基线：T-066~T-086 后 `pnpm verify:release` 全绿，包含 TypeScript、生产构建、565/565、移动烟测和 Chromium 样式烟测；raw bundle 自律预算 345 KiB，压缩包继续低于 300 KiB 硬上限。

待处理：

1. 继续保持完整自动门禁和 300 KiB 压缩包硬上限。
2. 下一候选：失败摘要出现/重试结束后的焦点保持；首页状态文案布局回归继续等待真实溢出证据。
