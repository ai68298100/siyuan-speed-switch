...
补充（2026-09-13）：T-232 修复 diagnostics capability spec 初始化顺序，并以直接 `require` 自检确认模块可加载；649/649 测试、TypeScript 与 diff 检查通过。
...
当前基线：`v0.16.39`（发布提交 `3d0d841`），开发头为本地 `main`；本地分支已包含未推送的性能/UI提交，T-071~T-121 已完成，T-122~T-230 已启动。正式发版继续后置。
