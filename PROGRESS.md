...
当前基线：`v0.16.39`（发布提交 `3d0d841`），开发头基于 `72e2deb`；本地 `main` 已包含两项未推送的性能/UI提交，T-071 正在形成新的本地提交。正式发版继续后置。
...
验证基线：T-066~T-071 后 `pnpm verify:release` 全绿，包含 TypeScript、生产构建、555/555、移动烟测和 Chromium 样式烟测；raw bundle 自律预算 340 KiB，压缩包继续低于 300 KiB 硬上限。

待处理：

1. T-072：月历翻月按钮与今日快捷定位，先完成键盘/触控可访问性契约。
2. T-073：面板可见性驱动读取，继续控制内核请求峰值与隐藏模块刷新。
3. T-074：缩略图可见优先缓存与移动端降载，建立命中率/内存边界观测。
4. 继续保持完整自动门禁和 300 KiB 压缩包硬上限。
5. 用户明确确认后，才执行 push、打 tag、创建 Release 等正式发版动作。
