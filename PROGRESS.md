...
当前基线：`v0.16.39`（发布提交 `3d0d841`），开发头为本地 `main`；本地分支已包含未推送的性能/UI提交，T-071~T-096 已完成。正式发版继续后置。
...
验证基线：T-066~T-101 后 `pnpm verify:release` 全绿，包含 TypeScript、生产构建、568/568、移动烟测和 Chromium 样式烟测；raw bundle 自律预算 345 KiB，压缩包约 298.7 KiB，条目级差异与硬上限余量均由 diagnostic 持续观测。

待处理：

1. 继续保持完整自动门禁和 300 KiB 压缩包硬上限。
2. 下一候选：确认宿主路径枚举端点后推进 T-102；真实窄屏验收按 D-042 后置，后续有意资源变更时先审阅并更新归档基线。
