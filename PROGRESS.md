...
验证基线：T-066~T-090 后 `pnpm verify:release` 全绿，包含 TypeScript、生产构建、565/565、移动烟测和 Chromium 样式烟测；raw bundle 自律预算 345 KiB，压缩包约 299 KiB，继续受 300 KiB 硬上限约束。

待处理：

1. 继续保持完整自动门禁和 300 KiB 压缩包硬上限。
2. 下一候选：首页刷新失败摘要有界快照；首页状态文案布局回归仍需真实溢出证据。
