...
补充（2026-09-14 v0.17 只读 Agent 审计）：完成 T-1253~T-1282 共 30 项，新增 `src/agent-readonly-audit.js`，统一能力名称/设备/effects/状态/原因归一化、重复与上限检查、脱敏失败回执、设备矩阵、version=1 快照和变化事件；`index.ts` 在生产注册前执行只读定义审计。新增 40 项回归，未开放任何新写入动作；全量测试 993/993，当前 `dist/index.js` 416051 bytes、`package.zip` 267685 bytes。
