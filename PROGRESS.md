...
本轮新增（2026-09-13）：journal-calendar 日历月视图（第 27 个内置，协议 v2.3 viewType=calendar 首个使用者，周一开头/日记日高亮/今天标记/点击直达）；商店卡片按尺寸成比例预览框（当前档高亮）；D-042 已录（跳过常规设备测试）。待办新想法：进度环渲染器（stat.arc）、周打卡行、卡片配色预设、农历、月历翻月、面板首开延迟首读。
UI 打磨（本轮）：折叠按钮文字→箭头图标；空状态淡色前缀；日历格悬停反馈；条目悬停左侧强调色条；stat hero 数字加大加粗；面板内嵌套外壳已去除（上轮）。本地 31 commit 待推送。
性能+交互优化（本轮）：组件面板打开时逐个错峰加载（80ms 间隔）——内核不再并发风暴；折叠体 max-height+opacity 平滑动画；进度条圆角端点；条目悬停过渡。新想法待办：月历翻月按钮、周打卡行（Duolingo 式）、进度环渲染器（stat.arc）、卡片配色预设、农历支持、缩略图缓存按需降载。
