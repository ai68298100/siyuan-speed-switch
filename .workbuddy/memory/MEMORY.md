...
13. **窗口无界的断言（`A[\s\S]*?B`）与目标无关，剥注释修不了**（第七类同族，D-395）：全仓 **38 文件 / 365 条**（未剥注释粗计）；剥注释后**以债清单为准**的精计为 **86 条 / 1 文件**（W1 已清零；剩余全部为 store-mobile-layout-contract：55 W2 否定式 + 17 W1 + 14 W3；已迁 36 个文件共 279 条，见 D-396），断言退化成"文件某处有 A、其后再某处有 B"（`index.scss` 有 104 处 `min-width: 0`，几乎恒真）。修法是**块级作用域断言**，不可机械替换——见 T-6280，债条目在 `SOURCE_SCAN_DEBT` 的 `css-window-scope` 组。**工具**：`node scripts/css-window-census.cjs`（**已剥注释**，且**按债清单分组**——债外命中不计入待迁，债内零命中会被点名，因为普查只认单行形态）、`node scripts/css-assertion-injector.cjs <测试文件> [样式表] [--old=<rev>] [--probe=…]`（判别力三件套自动化，见第 18/21 条）。
