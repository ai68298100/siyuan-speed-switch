...
23. **理由标签 / 白名单值必须被内容验证**（D-396 第三批，模式⑧第三次自我适用）：`tests/source-scan-coverage.test.cjs` 的 `DEBT_REASONS` 只校验"标签取自词汇表 + 说明 ≥20 字 + 不留死标签"，**从不校验标签与文件内容是否相符**——实测 `tests/kernel-endpoint-guard.test.cjs` 贴着 `css-window-scope`（"文件里的断言是无界窗口"）却一条窗口断言都没有（其 `[\s\S]*?` 是 `source.match(...)` 的区间截取，不在 assert 内），真债是读 `src/index.ts` 原文。**修法：给每个标签配一条可自动判定的内容判据**（T-6282）。
24. **`declaresIn` 目前只能按 `atDepth` 分层，不能区分 at-rule 种类**（T-6283）：所以"**窄屏分支**里必须有 X"只能退让成"任意 at-rule 里都算"。实测代价见 `store-box-sizing-contract` 的 `card keeps max width full`（`max-width: 100%` **只存在于 `@media (max-width: 560px)`**）：加 `{topLevel: true}` 会假红，不加则把声明挪去 `@media print` 也照样通过。**批量迁移 W2/W3 前先给 `parseRules` 记录 at-rule 身份**（收益最大）。
