...
13. **窗口无界的断言（`A[\s\S]*?B`）与目标无关，剥注释修不了**（第七类同族，D-395）：全仓 **38 文件 / 365 条**（未剥注释粗计；剥注释精计 **350 条**，其中选择器→声明类 240、非声明类 87、窄窗口 23；样板文件已迁完，见 D-396），断言退化成"文件某处有 A、其后再某处有 B"（`index.scss` 有 104 处 `min-width: 0`，几乎恒真；注入"从目标块删掉声明"仍通过）。修法是**块级作用域断言**（选择器 A 的块内声明 B），不可机械替换——见 T-6280，债条目在 `SOURCE_SCAN_DEBT` 的 `css-window-scope` 组。**待改条目的定位工具**：`node scripts/css-window-census.cjs [文件子串]`（**已剥注释**——否则"迁移说明注释里引用的旧写法"会被再数一遍，每迁一个文件就多一条幻影）。
