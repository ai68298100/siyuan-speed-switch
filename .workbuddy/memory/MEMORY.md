...
13. **窗口无界的断言（`A[\s\S]*?B`）与目标无关，剥注释修不了**（第七类同族，D-395）：全仓 **38 文件 / 365 条**（未剥注释粗计）；**T-6280 已完成（2026-09-17）**：365 条 / 38 文件窗口断言全部迁移为块级/锚定有界断言，css-window-scope 标签退役，普查工具保留作回归监测；23 批共删 31 条无信息测试、2 个真实产品修复（见 D-396）。T-6281 亦已完成：mobile-card-smoke 的 readSource 并入 readSourceText，smoke-harness 标签退役，剩余债仅 4 条登记过的合法例外（doc-comment 3 + json-data 1），断言退化成"文件某处有 A、其后再某处有 B"（`index.scss` 有 104 处 `min-width: 0`，几乎恒真）。修法是**块级作用域断言**，不可机械替换——见 T-6280，债条目在 `SOURCE_SCAN_DEBT` 的 `css-window-scope` 组。**工具**：`node scripts/css-window-census.cjs`（**已剥注释**，且**按债清单分组**——债外命中不计入待迁，债内零命中会被点名，因为普查只认单行形态）、`node scripts/css-assertion-injector.cjs <测试文件> [样式表] [--old=<rev>] [--probe=…]`（判别力三件套自动化，见第 18/21 条）。
...
27. **注入工具的 `-` 哨兵冲突**（D-396 第五批）：`css-assertion-injector.cjs` 探针的插入文本以 `-` 开头（`-webkit-hyphens: auto` 这类带前缀的标准属性）会被当成删除式哨兵、静默跳过；已支持 `\-` 转义为插入。"工具语法与被测对象词汇冲突"是一类新陷阱——新增语法时要排查与既有哨兵/分隔符的撞车。
28. **分组规则（逗号共选器）的记账口径**（D-396 第六批）：`A, B { X: … }` 共用一条规则时，迁移断言应各钉各的选择器（拆组丢一方会被抓）；审计面自检断言"每个受管**选择器**都被某条规则承载"（Set 扁平化比对），不能断言"规则条数 = 选择器个数"。注入验证时共享声明会使多条断言互为预期共因，属正当现象。
29. **手写窗口（变量截块）是普查的盲区**（D-396 第七批）：先用窗口正则截一块文本存进变量、再对变量做 match/doesNotMatch，普查（只认 assert 行内单行模式）扫不到——本仓实例 `cardHeadBlock` 以"下一个规则的类名"为锚截块，插删规则即漂移。"债清单归零"≠"窗口断言归零"，逐文件通读仍是最后一道工序。
30. **非空自检的下限不许钉死进度**（D-396 第八批）：给"随进度递减的集合"（债清单等）配"审计面 ≥N"自检时，N 必须按"比塌缩高、比最小合法终态低"校准（如 `>=3`），不能按历史时点的数量钉死（`>=35` 在债清到 34 时先于集合比对假红）。多失败混合时逐个归因：性能基准假失败先单独复跑再判。
31. **名不副实断言的三种处置**（D-396 第十批）：① CSS 支持名字 → 改写兑现；② 名字与现实都属真实意图但内容零信息 → 删除（注释写明真实契约由谁承载）；③ 名字错误但 CSS 有近似事实 → 换名改写。判据是"断言必须守护一个真实存在的约束"。遇到"声称 X 有某属性而 X 从未有过"时先全文件普查该属性——套模板会当场假红或锁死设计意图。
32. **前缀锚点是假绿的重灾区**（D-396 第十一批）：`\.sw-home-store__size[\s\S]*?X` 里 `__size` 前缀可匹配 `__sizes`/`__size-tile`——窗口断言的假绿不仅让门禁无效，还可能**掩护真实产品缺陷**（size tile 从未拿到 touch-action）。迁移时"块级断言当场假红"是发现机制：产品缺口修产品，不改门禁迁就。另：`declaresIn` 字符串选择器是完全相等，嵌套展开后的选择器带父级前缀，要用正则尾部锚定；查展开选择器用 `parseRules` 实跑，别用行扫描猜父级。
33. **复合 at-rule 的身份判别要精确等于原文**（D-396 第十五批）：`@media (max-width: 560px) and (prefers-reduced-motion: reduce)` 的条件链长度也是 1，判纯/复合分支须 `atRules[0]` 精确等于目标媒体原文（条数与深度都不可靠）。块级化还会显形 CSS 冗余（同一覆盖写两份），删被包含的一份属正当产品清理；工具局限（同选择器多规则删除探针只达首条）写入工具头，边界外用例用 md5 手工外科注入补验。
