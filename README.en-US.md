...
- Release gates pass: 5879 automated tests (173 test files), TypeScript, production build, mobile smoke, Chromium smoke, and `verify:release`; artifacts dist/index.js 631445 bytes, package.zip 320546 bytes.
...
**Test matrix**: `pnpm test` discovers all 173 `*.test.cjs` files under `tests/` and `tests/host/`, currently 5879 tests in total; the authoritative count is the command output. UI smoke tests run separately:
