...
- Release gates pass: 5890 automated tests (174 test files), TypeScript, production build, mobile smoke, Chromium smoke, and `verify:release`; artifacts dist/index.js 632957 bytes, package.zip 321021 bytes.
...
**Test matrix**: `pnpm test` discovers all 174 `*.test.cjs` files under `tests/` and `tests/host/`, currently 5890 tests in total; the authoritative count is the command output. UI smoke tests run separately:
