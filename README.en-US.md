...
- The development head passes 2923 automated tests, TypeScript, production build, mobile smoke, and Chromium UI smoke; artifact details are tracked in the release-readiness matrix.
...
**Test matrix**: `pnpm test` discovers all 135 `*.test.cjs` files under `tests/` and `tests/host/`, currently 2923 tests in total. UI smoke tests run separately:
...
pnpm test               # run every unit, contract, and host release test (currently 2923)
