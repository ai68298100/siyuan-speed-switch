...
**Test matrix**: `pnpm test` discovers all 212 `*.test.cjs` files under `tests/` and `tests/host/`, currently 6332 tests in total; the authoritative count is the command output. UI smoke tests run separately:
...
pnpm test               # run every unit, contract, and host release test (currently 6332)
...
pnpm verify:release     # local release-candidate gate (typecheck, reproducible two-build audit, 6332 tests, release/quality/integration audits, and all three UI smokes)
```
