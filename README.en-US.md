...
**Test matrix**: `pnpm test` discovers all 218 `*.test.cjs` files under `tests/` and `tests/host/`, currently 5904 tests in total; the authoritative count is the command output. UI smoke tests run separately:
...
pnpm test               # run every unit, contract, and host release test (currently 5904)
...
pnpm verify:release     # local release-candidate gate (typecheck, reproducible two-build audit, 5904 tests, release/quality/integration audits, and all three UI smokes)
```
