...
- The development head passes 5577 automated tests, TypeScript, production build, mobile smoke, and Chromium UI smoke; artifact details are tracked in the release-readiness matrix.
...
**Test matrix**: `pnpm test` discovers all 156 `*.test.cjs` files under `tests/` and `tests/host/`, currently 5577 tests in total. UI smoke tests run separately:
...
pnpm test               # run every unit, contract, and host release test (currently 5577)
