...
[![Version](https://img.shields.io/badge/version-0.27.0-blue)](./plugin.json) [![License: MIT](https://img.shields.io/badge/license-MIT-green)](./LICENSE) [![SiYuan](https://img.shields.io/badge/SiYuan-SiYuan_Note-ff5c67)](https://b3log.org/siyuan)
...
**Test matrix**: `pnpm test` discovers all 212 `*.test.cjs` files under `tests/` and `tests/host/`, currently 6327 tests in total; the authoritative count is the command output. UI smoke tests run separately:
...
pnpm test               # run every unit, contract, and host release test (currently 6327)
...
pnpm verify:release     # local release-candidate gate (typecheck, reproducible two-build audit, 6327 tests, release/quality/integration audits, and all three UI smokes)
```
...
The current version is `v0.27.0` (officially published on 2026-09-21; Release assets are built automatically by the workflow).
