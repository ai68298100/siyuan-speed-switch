...
- The development head passes 4889 automated tests, TypeScript, production build, mobile smoke, and Chromium UI smoke; artifact details are tracked in the release-readiness matrix.
...
**Test matrix**: `pnpm test` discovers all 144 `*.test.cjs` files under `tests/` and `tests/host/`, currently 4889 tests in total. UI smoke tests run separately:
...
pnpm test               # run every unit, contract, and host release test (currently 4889)
