...
- Release gates pass: 5884 automated tests (173 test files), TypeScript, production build, mobile smoke, Chromium smoke, and `verify:release`; artifacts dist/index.js 632957 bytes, package.zip 320945 bytes.
...
**Test matrix**: `pnpm test` discovers all 173 `*.test.cjs` files under `tests/` and `tests/host/`, currently 5884 tests in total; the authoritative count is the command output. UI smoke tests run separately:
