...
**Test matrix**: `pnpm test` discovers all 93 `*.test.cjs` files under `tests/` and `tests/host/`, currently 683 tests in total. UI smoke tests run separately:
...
pnpm test               # run every unit, contract, and host release test (currently 683)
