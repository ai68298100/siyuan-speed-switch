...
- Desktop first- and second-panel sizes now open at an adaptive screen ratio by default (90%, adjustable 50–100 in settings); Appearance settings gain a "Panel size mode" (adaptive screen ratio / fixed size / fullscreen) that replaces the standalone fullscreen toggle formerly in Behavior; the in-dialog fullscreen button still toggles temporarily.
- One-time quick-action defaults migration: after upgrading, entries auto-written into the bar by older releases (including third-party plugin entries) reset to the default search, journal, and settings three; manual curation made afterwards is kept permanently.
- Fixed the mobile top-bar switcher entry overlapping the host's trailing buttons (close/more): the entry is now inserted before the host's trailing button, and misplaced buttons appended by older releases are repositioned automatically.
- Release bundle optimization: the esbuild target moved to `es2020` (no more async/await downleveling for modern WebViews) and license comments are stripped from the minified output, bringing `dist/index.js` back inside the mobile performance budget (~229KiB; LICENSE still ships in full at the top of the artifact).
- Release archive audit tests now use a dependency-free zip parser (`tests/host/lib/zip.cjs`) instead of shelling out to the host `tar`, so they reproduce reliably under Git Bash / GNU tar as well.
...
- Fresh installs keep Search, Journal, and Settings as the three configurable defaults, plus a fixed Second panel entry. Third-party actions remain available under Add action and no longer occupy the bar automatically; existing user configurations remain valid.
...
- **Quality gates**: tsc 0 errors, build OK (currently about 237 KiB package.zip; hard ceiling 300 KiB), entry bundle under 230 KiB, zero `console` / `debugger` / `alert` / `@ts-ignore` residuals, the only `as any` occurrence is in a comment (not code).
...
**Test matrix**: `pnpm test` discovers all 89 `*.test.cjs` files under `tests/` and `tests/host/`, currently 667 tests in total. UI smoke tests run separately:
...
pnpm test               # run every unit, contract, and host release test (currently 580)
