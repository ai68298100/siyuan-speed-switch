...
**Test matrix**: `pnpm test` discovers all 94 `*.test.cjs` files under `tests/` and `tests/host/`, currently 689 tests in total. UI smoke tests run separately:
...
pnpm test               # run every unit, contract, and host release test (currently 689)
