...
**Test matrix**: `pnpm test` discovers all 222 `*.test.cjs` files under `tests/` and `tests/host/`, currently 6011 tests in total; the authoritative count is the command output. UI smoke tests run separately:
...
pnpm test               # run every unit, contract, and host release test (currently 6011)
...
pnpm verify:release     # local release-candidate gate (typecheck, reproducible two-build audit, 6011 tests, release/quality/integration audits, and all four UI smokes)
```
