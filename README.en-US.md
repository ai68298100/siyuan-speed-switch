...
[![Version](https://img.shields.io/badge/version-0.32.0-blue)](./plugin.json) [![License: MIT](https://img.shields.io/badge/license-MIT-green)](./LICENSE) [![SiYuan](https://img.shields.io/badge/SiYuan-SiYuan_Note-ff5c67)](https://b3log.org/siyuan)
...
**Test matrix**: `pnpm test` discovers all 225 `*.test.cjs` files under `tests/` and `tests/host/`, currently 6064 tests in total; the authoritative count is the command output. UI smoke tests run separately:
...
pnpm test               # run every unit, contract, and host release test (currently 6020)
...
pnpm verify:release     # local release-candidate gate (typecheck, reproducible two-build audit, 6020 tests, release/quality/integration audits, and all four UI smokes)
```
...
The current version is `v0.32.0` (pinyin matching, jump back/forward, Essentials layer, filter chips, workbench, icon catalog and skin layer; Release assets are built automatically by the workflow).
