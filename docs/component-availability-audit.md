...
> 2026-09-17 补充：本文是 2026-09-13 的**历史快照**，其中“30 个内置组件”等计数保持原样。
> 当期口径为 `category === "siyuan"` 的内置组件 47 个（含 5 个小驴打卡桥接组件）、
> 第三方目录组件 1 个（`checkin-summary`）；来源分组与桥接方案见 `docs/adr/0057-widget-source-and-store-grouping.md`。

## 结论摘要

- 当前商店包含 **30 个内置组件**，另有 **1 个第三方目录组件**（`checkin-summary`）。
- 30 个内置组件都有真实 adapter 注册；`checkin-summary` 没有本插件内 adapter，必须由 `siyuan-checkin` 注册后才会进入“可用组件”。
- 当前自动门禁为 **1711/1711 通过**。这证明协议、归一化、超时、缓存和渲染边界成立，不等同于每个组件都已在真实思源数据上验收。
...
| `today-tasks` 今日待办 | B/C | `/api/query/sql` 查询今日日记中的规范任务列表项（`type=i/subtype=t`），点击受控更新勾选；可选近期全库扫描 | 默认以 `custom-dailynote-YYYYMMDD` 属性定位思源日记，并兼容 `YYYY-MM-DD` 标题，不再依赖当前页签 | 真实验收自定义日记路径、任务块和全库模式；后续支持显式到期日期 |
...
| `external-anime-bangumi` 每日放送 | B/C | Bangumi legacy `/calendar` 兼容接口 | 用户添加后才联网；整周结果按本地星期筛选，支持今日/明日/本周；官方封面域名三层白名单、懒加载、30 分钟缓存和来源署名均有自动回归 | 真实宿主验证代理网络、封面防盗链、深浅主题和窄侧栏；官方 v0 出现等价接口后迁移 |
