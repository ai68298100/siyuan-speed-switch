...
Current build: `dist/index.js` 803239 bytes; `dist/index.css` 150880 bytes; `package.zip` 382231 bytes (v0.23.5 Release 资产实测，与本地构建一致)；`icon.png` 160x160 18376 bytes.
...
| 生产产物与包体 | 已通过 | 当前构建 `dist/index.js` 803239 bytes（832 KiB 自律线内，余量 48729 bytes，ADR 0062）；`dist/index.css` 150880 bytes；`package.zip` 382231 bytes（512 KiB 硬上限余量 142057 bytes，v0.23.5 Release 资产实测 = 本地构建）；zip 单条目压缩预算 180→192→224 KiB（T-6357/T-6400 / ADR 0059，当前 `index.js` 压缩后 212645 bytes，余量 16731 bytes）；负向验证临时回退 190 KiB 时目标门禁精确 FAIL，恢复 224 KiB 后通过且 SHA-256 一致。后续以功能、性能和交互为优先，体积仅防失控增长 |
