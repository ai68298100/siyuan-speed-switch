...
| 生产产物与包体 | 已通过 | 当前构建 `dist/index.js` 820502 bytes（832 KiB 自律线内，余量 31466 bytes，ADR 0062）；`dist/index.css` 150880 bytes；`package.zip` 386923 bytes（512 KiB 硬上限余量 137365 bytes）；zip 单条目压缩预算 180→192→224→256 KiB（T-6357/T-6400 / ADR 0059 / ADR 0065，当前 `index.js` 压缩后 217596 bytes，余量 44548 bytes）；预算、余量和快照漂移统一由 `scripts/release-readiness-metrics.cjs` 提供；预算、余量和快照漂移统一由 `scripts/release-readiness-metrics.cjs` 提供；负向验证临时回退 190 KiB 时目标门禁精确 FAIL，恢复 224 KiB 后通过且 SHA-256 一致。后续以功能、性能和交互为优先，体积仅防失控增长 |
