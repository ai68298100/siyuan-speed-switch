...
<!-- Current artifact snapshot: dist/index.js 804014 bytes; dist/index.css 150880 bytes; package.zip 382578 bytes; icon.png 160x160 18376 bytes; mobile self-regulation line 832 KiB (ADR 0062); archive ceiling 512 KiB. -->

Current build: `dist/index.js` 804014 bytes; `dist/index.css` 150880 bytes; `package.zip` 382578 bytes (本地构建实测；相对 v0.23.5 Release 资产的差异来自本轮中文名称更新)；`icon.png` 160x160 18376 bytes.
...
| 生产产物与包体 | 已通过 | 当前构建 `dist/index.js` 804014 bytes（832 KiB 自律线内，余量 47954 bytes，ADR 0062）；`dist/index.css` 150880 bytes；`package.zip` 382578 bytes（512 KiB 硬上限余量 141710 bytes）；zip 单条目压缩预算 180→192→224 KiB（T-6357/T-6400 / ADR 0059，当前 `index.js` 压缩后 212872 bytes，余量 16504 bytes）；预算、余量和快照漂移统一由 `scripts/release-readiness-metrics.cjs` 提供；负向验证临时回退 190 KiB 时目标门禁精确 FAIL，恢复 224 KiB 后通过且 SHA-256 一致。后续以功能、性能和交互为优先，体积仅防失控增长 |
