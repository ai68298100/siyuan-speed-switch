...
<!-- Current artifact snapshot: dist/index.js 660917 bytes; dist/index.css 147573 bytes; package.zip 332271 bytes; icon.png 160x160 18376 bytes; mobile self-regulation line 768 KiB; archive ceiling 512 KiB. -->

Current build: `dist/index.js` 660917 bytes; `dist/index.css` 147573 bytes; `package.zip` 332271 bytes; `icon.png` 160x160 18376 bytes.

评估日期：2026-09-18（v0.21.0 发布后开发头）；真实宿主补充验收仍按兼容性任务持续跟进。

| 检查项 | 状态 | 证据/剩余动作 |
| --- | --- | --- |
| TypeScript、自动测试、UI smoke | 已通过 | 完整测试 **5987/5987**（2026-09-18 复核；含 14 项手机端图标兜底与布局契约、4 项文档集恢复报告契约、16 项缩略图缓存读取侧归一化契约：9 项纯函数 + 7 项演练管道/分类、6 项存储兼容矩阵文档契约、2 项对外文档版本承诺一致性契约、3 项源码扫描剥注释覆盖率门禁 + 1 项债清单理由标签内容判据门禁（T-6282）、14 项 CSS 块级作用域断言辅助自测、4 项根目录文档体积与归档契约门禁、小驴打卡桥接 15 项 + 组件协议 v2.4 来源字段 17 项 + 来源分组模型 13 项 + 打卡注册模板 10 项（T-6298 / ADR 0057）、**命令注册守卫 11 项（T-6301 / issue #1：languages 门控 + 安全注册降级语义 + index.ts 源码契约 + 探测器自检，注入裸 `this.addCommand(` 精确 FAIL 后 md5 还原）**、**RSS/Atom 订阅组件 17 项（T-6303：配置钳制、URL 门禁、缓存/陈旧回退、RSS 2.0 与 Atom 双格式有界解析、实体单轮解码、超限 parse_failed、快照形状，排序注入精确 FAIL 后 md5 还原）**）；TypeScript、移动端 smoke 70 项、Chromium smoke、手机布局门禁（3 断言，含宿主兜底探针）、生产图隔离测试与 6 项注释剥离辅助自测通过 |
...
| 生产产物与包体 | 已通过 | 当前构建 `dist/index.js` 660917 bytes（768 KiB 自律线内，余量 125515 bytes）；`dist/index.css` 147573 bytes；`package.zip` 332271 bytes（512 KiB 硬上限余量 192017 bytes）；zip 单条目压缩预算 168→180 KiB（T-6298 / ADR 0057：`index.js` 压缩后 164015→175727 bytes，纯本地只读增量，按 D-353/D-366/D-386 先例上调，负向验证回退到 168 KiB 仍精确 FAIL）；T-6301/T-6303 增量在此预算内，未再上调 |
