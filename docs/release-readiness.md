...
| 生产产物与包体 | 已通过 | 当前构建 `dist/index.js` 799847 bytes（832 KiB 自律线内，余量 52121 bytes，ADR 0062）；`dist/index.css` 149362 bytes；`package.zip` 378386 bytes（512 KiB 硬上限余量 145902 bytes）；zip 单条目压缩预算 180→192→224 KiB（T-6357/T-6400 / ADR 0059，当前 `index.js` 压缩后 211746 bytes，余量 17630 bytes）；负向验证临时回退 190 KiB 时目标门禁精确 FAIL，恢复 224 KiB 后通过且 SHA-256 一致。后续以功能、性能和交互为优先，体积仅防失控增长 |
...
| 版本元数据 | 已通过 | `0.23.3` 已同步 `package.json`、`plugin.json`、中英文 README |
