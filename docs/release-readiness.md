...
<!-- Current artifact snapshot: dist/index.js 802089 bytes; dist/index.css 150880 bytes; package.zip 379095 bytes; icon.png 160x160 18376 bytes; mobile self-regulation line 832 KiB (ADR 0062); archive ceiling 512 KiB. -->

Current build: `dist/index.js` 802089 bytes; `dist/index.css` 150880 bytes; `package.zip` 379095 bytes; `icon.png` 160x160 18376 bytes.
...
| 生产产物与包体 | 已通过 | 当前构建 `dist/index.js` 802089 bytes（832 KiB 自律线内，余量 52121 bytes，ADR 0062）；`dist/index.css` 150880 bytes；`package.zip` 379095 bytes（512 KiB 硬上限余量 145902 bytes）；zip 单条目压缩预算 180→192→224 KiB（T-6357/T-6400 / ADR 0059，当前 `index.js` 压缩后 212253 bytes，余量 17123 bytes）；负向验证临时回退 190 KiB 时目标门禁精确 FAIL，恢复 224 KiB 后通过且 SHA-256 一致。后续以功能、性能和交互为优先，体积仅防失控增长 |
