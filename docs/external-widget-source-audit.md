...
首批采用分层接入：本地时间直接内置；天气、节假日与 Bangumi 先做免 Key 适配；热榜和新闻要求可配置/自部署端点；TMDB 要求用户 API Key；使用时长仅连接用户本机 ActivityWatch。活动窗口读取暂不接入生产。

| 候选组件 | 来源 | 许可/条款 | 凭据 | 建议状态 | 关键边界 |
| --- | --- | --- | --- | --- | --- |
| 时间与日期 | 浏览器 `Intl.DateTimeFormat` | 平台能力 | 无 | 已内置 | 完全离线，每分钟刷新，页面隐藏时暂停 |
| 近期天气 | [Open-Meteo](https://github.com/open-meteo/open-meteo) | AGPL-3.0；API 另受服务条款约束 | 无 | 下一批 | 只发送坐标/城市；缓存 15–30 分钟；允许手动城市，定位必须主动授权 |
...
| 中国节假日 | [holiday-cn](https://github.com/NateScarlet/holiday-cn) | MIT | 无 | 下一批 | 每年静态 JSON；本地缓存；年份交界需同时检查相邻年份 |
...
2. Open-Meteo 天气：城市手选优先，15 分钟缓存，三端卡片渲染。
3. holiday-cn：节假日/调休标记复用现有月历，不新增第二套月历。
