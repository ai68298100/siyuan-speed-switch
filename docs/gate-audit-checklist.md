...
| **365 条无界窗口断言**（38 文件，T-6280） | 均可被同文件他处的声明满足（D-395 注入实证）。修法是 CSS 块级作用域断言助手 + 逐文件迁移；**不能机械替换**，需先做块解析自测与"删掉目标声明必须 FAIL"的判别力验证。已完成 `SOURCE_SCAN_DEBT` 登记（`css-window-scope` 组），迁移一条删一条 |
