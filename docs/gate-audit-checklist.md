...
| **350 条无界窗口断言**（37 文件，T-6280） | 均可被同文件他处的声明满足（D-395 注入实证）。修法是 CSS 块级作用域断言助手 + 逐文件迁移；**不能机械替换**，需先做块解析自测与"删掉目标声明必须 FAIL"的判别力验证。助手与机制已就位（`tests/css-block-scan.cjs`，含 `{topLevel: true}` 作用域，D-396），`store-width-contract` 已作为样板迁完（15 条 → 10 条真事实，10/10 判别力验证通过）；其余按 `SOURCE_SCAN_DEBT` 的 `css-window-scope` 组逐文件推进，迁一条删一条 |
