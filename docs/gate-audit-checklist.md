...
| **319 条无界窗口断言**（34 文件，T-6280） | 均可被同文件他处的声明满足（D-395 注入实证）。修法是 CSS 块级作用域断言助手 + 逐文件迁移；**不能机械替换**，需先做块解析自测与"删掉目标声明必须 FAIL"的判别力验证。助手与机制已就位（`tests/css-block-scan.cjs`，含 `{topLevel: true}` 作用域，D-396），判别力验证已工具化（`scripts/css-assertion-injector.cjs`，自动跑"精确 FAIL + 兄弟不受波及 + 旧写法仍绿"三件套，规则式与 `deepEqual` 式断言用 `--probe`），普查口径已改为以债清单为准（`scripts/css-window-census.cjs`，债内/债外分组并点名零命中文件）；已迁 3 个文件（`store-width-contract` 15、`store-font-weight-contract` 12、`store-box-sizing-contract` 12，共 39 条，判别力逐个验证），其余按 `SOURCE_SCAN_DEBT` 的 `css-window-scope` 组逐文件推进，迁一条删一条。**注意**：是否漏检取决于右侧模式在同文件里的稀有度，**必须逐条注入实测**（现有两个方向相反的样本：8/8 全漏检 vs 12 条里仅 2 条漏检） |
| **`SOURCE_SCAN_DEBT` 的理由标签从不被验证**（T-6282） | `DEBT_REASONS` 只校验"标签取自词汇表 + 说明 ≥20 字 + 不留死标签"，**从不校验标签与文件内容是否相符**。实测 `kernel-endpoint-guard.test.cjs` 贴着 `css-window-scope`，却一条窗口断言都没有（其 `[\s\S]*?` 是 `source.match(...)` 里的区间截取，不在 assert 内），它真正的债是读 `src/index.ts` 原始文本。修法：给每个标签配一条可自动判定的内容判据，不符即 FAIL。属"白名单值必须被断言"（模式⑧）的第三次自我适用 |
| **`declaresIn` 无法区分 at-rule 种类**（T-6283） | `parseRules` 只记录 `atDepth`（层数），因此"**窄屏分支**里必须有 X"只能退让成"任意 at-rule 里都算"。实测代价：`store-box-sizing-contract` 的 `card keeps max width full` 的声明只存在于 `@media (max-width: 560px)`，既不能加 `{topLevel: true}`（会假红），又不能精确到该分支（把声明挪去 `@media print` 断言仍会通过）。W2 87 条与 W3 17 条预计大量涉及窄屏/打印分支，**批量迁移前升级收益最大** |
