...
| 项 | 说明 |
| --- | --- |
| **版本/tag 约束共享模块已完成（T-6661，2026-09-19）** | `scripts/release-version-contract.cjs` 统一稳定 semver、`v` 前缀 tag 和 package/plugin/tag 一致性；`scripts/release-version-preflight.cjs` 接入 GitHub Release workflow，rollback/release-docs/github-release/release-quality 测试与 release batch audit 复用同一实现；错误 tag 的 CLI 与测试负向验证均已执行 |
| **构建产物两次构建对比已完成（T-6659，2026-09-19）** | `scripts/reproducible-build-audit.cjs` 已在 CI/Release 连续执行两次生产构建，并对 `dist/index.js`、`dist/index.css`、`package.zip` 做字节数和 SHA-256 比对；本机实测 3/3 一致。 |
