...
「组件面板」是思源笔记插件「小驴雷切 (LvSpeed Switch)」提供的聚合主页：每个插件都可以把自己的数据注册为一个**小组件**，出现在面板的组件商店里，由用户自由添加、摆放到 12 列网格画布上。

本指南面向插件开发者，说明如何让自己的插件出现在组件商店里。

## 快速接入（三步）

在你的插件 `onload()` 中调用小驴雷切的公开 API：
...
`read` 返回的条目可携带 `command: "插件名::命令key"`，点击该条目时由小驴雷切代为执行对应插件命令——不需要自己处理点击跳转逻辑：
...
如果宿主支持思源 Agent，小驴雷切会把已注册且可读取的组件纳入 `home-widget-snapshot`：
...
2. 组内卡片按 `order` **升序**排列（同 `order` 再按 `moduleId` 字典序）——你可以
   直接决定用户先看到哪个组件；不填 `order` 时退化为按 `moduleId` 字典序，
   顺序依旧确定、不随枚举顺序漂移；
3. 组头显示来源图标、**已添加 x/y** 计数与**全选本组 / 取消本组**；
4. 来源名、`pluginId` 与 `collection` 进入卡片搜索文本——搜插件名或子系列名
   都能命中整套组件；
5. `collection`（子系列）**只进搜索文本，不做组内二级分组**：目前还没有真实的多
   系列数据能验证二级小标题是否好用，先不引入没有数据支撑的 UI。

`version` 与 `homepage` 会被归一化并随来源一起透传，但当前**不参与商店呈现**——
宁可少显示，也不挂消费不到的字段。
...
自 v0.17 起，小驴雷切额外注册只读能力 `workspace-runtime-registry-diagnostics`：返回有界的组件运行时计数快照（注册数/会话数/队列游标），无输入、无正文内容、无宿主异常透出；Agent 可用它在诊断对话中确认组件面板的基础设施状态。组件本身的生命周期事件（失效/恢复）不影响该计数以外的任何输出。
