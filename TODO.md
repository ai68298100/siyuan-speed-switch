...
\}` 冗余 + 逐字重复）；**两处新发现**：① card 的 transition 覆盖在两个 reduce 块各写一份（4848 块被 4944 分组规则完全包含）——块级化显形后**删被包含的 CSS**；② 复合分支 `@media (max-width: 560px) and (prefers-reduced-motion: reduce)` 的条件链也是一条，判别纯/复合分支必须用**条件链精确等于**而非条数；工具局限实录：同选择器多规则的删除探针只达首条（hover transform 断言用 md5 手工补验），已写入工具头。35→33 测试）。⑰ 第十六批 `store-scroll-contract.test.cjs` 迁完（7 条 → 5 条块级 + 1 条逐规则守卫（grid 全部规则禁 `overflow: hidden`）；**另删 1 条手写窗口**：`gridBlock` 按"第一个块尾"截块再做否定检查（第二例变量截块，锚点脆弱）+ 1 条迁移后逐字重复；5/5 + 探针 1 次全过）。⑱ 第十七批 `store-preview-disclosure-contract.test.cjs` 迁完（8 条 → 9 条块级——`&__meta`/`&__meta-chip` 按展开选择器、两个 tone 类（is-network/is-privacy）收紧到具名块；TS 侧 1 条按 home-store-ui.ts:112 原文改精确调用断言并手工负向验证（md5 还原）；TS 裸读改走 readSourceText；9/9 提取断言全过）。**⑱ 记账修正（本批自查发现）**：T-6278 时代写入的 W2「债内 87 条」从未随批次更新（实际批次 8 起为 85），且第十四/十五批提交信息里的 W1/W2 拆分有误（总数正确）——本批起以普查工具的分档输出为准逐项核对，W1/W2/W3 三数之和必须等于合计。**剩余**：形态分布（`node scripts/css-window-census.cjs`，**口径=债清单**）为 W1「选择器→声明」债内 71 条 / W2「选择器→非声明」债内 85 条（需人审：比较两属性、否定式窗口、`url(`/`javascript` 依赖检查；正确形态是"逐规则检查"——先取声明了 A 的规则，再断言这些规则的声明不含 B）、W3 窄窗口 `[^}]*` 债内 17 条，合计 **172 条 / 20 文件**（已迁 `store-width-contract` 15、`store-font-weight-contract` 12、`store-box-sizing-contract` 12、`store-line-height-contract` 14、`store-hyphenation-contract` 14、`store-font-metrics` 11、`store-typography-contract` 11、`store-numeric-typography` 13、`store-selection-contract` 11、`store-text-wrap-pretty` 11、`store-focus-navigation-contract` 10、`store-stacking-contract` 9、`store-preview-context-contract` 10、`store-word-break-contract` 8、`store-motion-accessibility` 10、`store-scroll-contract` 7、`store-preview-disclosure-contract` 8 条；计数变化另含助手自测 +2 与删除 22 条——纯冗余 18 + 假绿 4）。逐文件迁移，每迁完一个文件就删掉它对应的 `SOURCE_SCAN_DEBT` 条目。
