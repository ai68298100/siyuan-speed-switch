...
- [~] T-6280（**进行中**，第一批已交付，见 D-396）CSS 块级作用域断言助手与窗口断言迁移：全仓仍有用 `A[\s\S]*?B` 形式、"窗口无界"的断言（未剥注释的**粗计 365 条**、38 个文件；剥注释后**精计 350 条**，见下方"剩余"）。B1/B2 已证明这类断言与目标规则无关（从目标块删掉声明后仍绿）。**已完成**：① 助手 `tests/css-block-scan.cjs`（块解析 + SCSS 嵌套展开 + at-rule 透明 + 先剥注释）与 `tests/css-block-scan.test.cjs` 12 项自测；② `{topLevel: true}` 作用域——迁移中发现**新变体**：同名覆盖规则（`@media (max-width: 560px)`）会替基础规则满足断言，删掉基础声明也不失败，必须只认未被 at-rule 包裹的规则；③ `readSourceFile()` 读取入口（限定源码扩展名，防止变成绕开裸读登记的后门）；④ 样板 `store-width-contract.test.cjs` 迁完（15 条 → 10 条块级 + 5 条否定断言，10/10 判别力验证通过），对应债条目已删。**剩余**：形态分布（精计，`node scripts/css-window-census.cjs`）为 W1「选择器→声明」228 条 / W2「选择器→非声明」87 条（需人审：比较两属性、否定式窗口、`url(`/`javascript` 依赖检查；正确形态是"逐规则检查"——先取声明了 A 的规则，再断言这些规则的声明不含 B）、W3 窄窗口 `[^}]*` 23 条，合计 **338 条 / 36 文件**（已迁 `store-width-contract` 15 条、`store-font-weight-contract` 12 条）。逐文件迁移，每迁完一个文件就删掉它对应的 `SOURCE_SCAN_DEBT` 条目，并对每条改写做"删掉目标声明 → 必须（且只有它）FAIL"的注入；**不能机械替换**。**工具**：`scripts/css-window-census.cjs [文件子串]` 列出待改条目（**已剥注释**——否则迁移说明注释里引用的旧写法会被再数一遍）。**注意**：窗口断言是否漏检取决于右侧模式的稀有度，**不能靠猜**——实测 `store-font-weight-contract` 12 条里有 2 条意外地并不漏检，必须逐条注入实测。
