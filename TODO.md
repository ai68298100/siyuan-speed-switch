...
- [ ] T-6282（**待办**，T-6280 第三批捞出的**门禁缺陷**）`SOURCE_SCAN_DEBT` 的**理由标签从不被验证**——`DEBT_REASONS` 只校验"标签取自词汇表 + 说明 ≥20 字 + 每个标签都被用到"，**从不校验标签与文件内容是否相符**。实测 `tests/kernel-endpoint-guard.test.cjs` 贴着 `css-window-scope`（说明写着"文件里的断言是无界窗口"），但该文件**一条窗口断言都没有**：它唯一那处 `[\s\S]*?` 是 `source.match(/KERNEL_ENDPOINTS\s*=\s*new Set\(\[([\s\S]*?)\]\)/)` 里的**区间截取**，不在 assert 内；它真正的债是"读 `src/index.ts` **原始文本**"（注释掉的 `case "/api/…":` 照样会被 `dispatchCases()` 数到），修法是改走 `readSourceText`，与作用域无关。**处置**：给每个标签配一条可自动判定的**内容判据**（`css-window-scope` ⇔ 剥注释后确实存在窗口断言形态；`doc-comment-contract`/`json-data`/`smoke-harness` 同理），标签与内容不符即 FAIL；顺带把该文件改走 `readSourceText` 并重新归类。**这正是清单"白名单值必须被断言"那一条的又一次自我适用**（本仓第三次：`sanitizeAllowlist`、`DEBT_REASONS`、现在这条）。
