...
- [ ] T-6283（**待办**，T-6280 第三批暴露的必要升级）`parseRules` 记录 **at-rule 身份**（当前只有 `atDepth` 层数），让"**窄屏分支**里必须有 X"可被精确断言。触发场景：`store-box-sizing-contract` 的 `card keeps max width full`——`max-width: 100%` **只声明在 `@media (max-width: 560px)` 分支里**，`{topLevel: true}` 会假红、不带又退化成"任意 at-rule 里都算"（现在退让成 `atDepth > 0`，若有人把它挪去 `@media print` 断言仍会通过）。W2 87 条与 W3 17 条里预计大量涉及窄屏/打印分支，**在批量迁移前升级收益最大**；升级后需同步 `tests/css-block-scan.test.cjs` 自测与 `docs/gate-audit-checklist.md`。
