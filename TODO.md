...
\}` 冗余 + 逐字重复）；**两处新发现**：① card 的 transition 覆盖在两个 reduce 块各写一份（4848 块被 4944 分组规则完全包含）——块级化显形后**删被包含的 CSS**；② 复合分支 `@media (max-width: 560px) and (prefers-reduced-motion: reduce)` 的条件链也是一条，判别纯/复合分支必须用**条件链精确等于**而非条数；工具局限实录：同选择器多规则的删除探针只达首条（hover transform 断言用 md5 手工补验），已写入工具头。35→33 测试）。**剩余**：形态分布（`node scripts/css-window-census.cjs`，**口径=债清单**）为 W1「选择器→声明」债内 85 条 / W2「选择器→非声明」债内 87 条（需人审：比较两属性、否定式窗口、`url(`/`javascript` 依赖检查；正确形态是"逐规则检查"——先取声明了 A 的规则，再断言这些规则的声明不含 B）、W3 窄窗口 `[^}]*` 债内 17 条，合计 **187 条 / 22 文件**（已迁 `store-width-contract` 15、`store-font-weight-contract` 12、`store-box-sizing-contract` 12、`store-line-height-contract` 14、`store-hyphenation-contract` 14、`store-font-metrics` 11、`store-typography-contract` 11、`store-numeric-typography` 13、`store-selection-contract` 11、`store-text-wrap-pretty` 11、`store-focus-navigation-contract` 10、`store-stacking-contract` 9、`store-preview-context-contract` 10、`store-word-break-contract` 8、`store-motion-accessibility` 10 条；计数变化另含助手自测 +2 与删除 20 条——纯冗余 16 + 假绿 4）。逐文件迁移，每迁完一个文件就删掉它对应的 `SOURCE_SCAN_DEBT` 条目。
