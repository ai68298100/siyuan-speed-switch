...
\}\s*
\s*\.sw__home-source-health/` 截块再做否定窗口，锚点是**下一个规则的类名**，中间插删规则即漂移——同族病；合计 12 条 → 9 条块级 + 2 条逐规则否定（声明 balance 的规则全文件恰 2 条）+ 删除 1 条纯冗余并删掉手写截块变量；收紧 1 条文件级断言到块级；9/9 + 探针 4 次（覆盖两条 balance 规则）全过）。**剩余**：形态分布（`node scripts/css-window-census.cjs`，**口径=债清单**）为 W1「选择器→声明」债内 165 条 / W2「选择器→非声明」债内 87 条（需人审：比较两属性、否定式窗口、`url(`/`javascript` 依赖检查；正确形态是"逐规则检查"——先取声明了 A 的规则，再断言这些规则的声明不含 B）、W3 窄窗口 `[^}]*` 债内 17 条，合计 **269 条 / 30 文件**（已迁 `store-width-contract` 15 条、`store-font-weight-contract` 12 条、`store-box-sizing-contract` 12 条、`store-line-height-contract` 14 条、`store-hyphenation-contract` 14 条、`store-font-metrics` 11 条、`store-typography-contract` 11 条；计数变化另含助手自测 +2 与删除 7 条纯冗余）。逐文件迁移，每迁完一个文件就删掉它对应的 `SOURCE_SCAN_DEBT` 条目。
