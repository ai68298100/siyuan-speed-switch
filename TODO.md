...
## T-6272~T-6273 v0.20 存储兼容性审计与叙述纠错（2026-09-16，已完成）

**本组终态**：`dist/index.js` 614133 bytes、`dist/index.css` 145639 bytes、`package.zip` 313129 bytes——**与上一批逐字节一致**（`5768→5774`、`164→165` 均为等长替换，JSDoc 注释被压缩器剥离）；测试 5774 项（165 文件）；`pnpm verify:release` 全链绿（5774/5774 + 三套 UI 门禁共 80 项 PASS）。
...
- [x] T-6274 文档与快照同步：README 双语 5768→5774 项、164→165 文件；`docs/release-readiness.md` 测试数同步（产物快照因等长替换**无需变更**）。**门禁再次生效**：新增测试文件后 `README test-file count matches the discovered host test matrix` 立即失败（期望 165），证明该门禁真的在守护文件数。
