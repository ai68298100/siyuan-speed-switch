...
- [ ] T-6280（**待办**，已在门禁里显式登记为债）CSS 块级作用域断言助手与窗口断言迁移：全仓仍有用 `A[\s\S]*?B` 形式、"窗口无界"的断言 **365 条，散在 38 个文件**（`store-mobile-layout-contract` 单文件 83 条最多）。B1/B2 已证明这类断言与目标规则无关（从目标块删掉声明后仍绿）。修法是写一个块级作用域助手（按 `selector {...}` 配对取块，含 SCSS 嵌套），把 `/A[\s\S]*?B/` 改写成"选择器 A 的块内声明了 B"，再逐文件迁移；**不能机械替换**——需先做块解析的自身测试与判别力验证（对每条改写做"删掉目标声明 → 必须 FAIL"的注入）。完成后从 `SOURCE_SCAN_DEBT` 的 `css-window-scope` 组逐条删除。
