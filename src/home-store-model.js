...
function resolveHomeConfigSection(moduleId, fieldKey) {
    ...
    if (key === "limit" || key === "maxItems" || key === "temperatureUnit" || key === "sortBy" || key === "groupByDocument" || key === "primaryMetric" || key === "metric" || key === "density" || key === "weekStart" || key === "hourFormat" || key === "mode" || key.startsWith("show")) return "display";
    return "options";
}
