...
**测试矩阵**：`pnpm test` 自动发现 `tests/` 与 `tests/host/` 下的 `*.test.cjs` 文件，当前共 1509 项测试（102 个测试文件）；UI 冒烟测试单独执行：
