...
> 当前开发策略：本版已通过类型检查、生产构建、993 项自动测试、移动端与 Chromium UI 烟测并完成正式发布；收藏/置顶/分组容量边界已锁定（512/64/64），路径筛选真实宿主能力、窄侧栏和 Android 真机验收仍作为后续兼容性补充。
...
**测试矩阵**：`pnpm test` 自动发现 `tests/` 与 `tests/host/` 下的 `*.test.cjs` 文件，当前共 993 项测试（102 个测试文件）；UI 冒烟测试单独执行：
