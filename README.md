...
- **最近打开记录**：桌面端顶部时钟入口保留最近打开的文档；仍在页签中的记录可直接切换，已关闭的文档可点击重新打开。
...
全库请求使用 180ms 防抖、容量受限的内存缓存、请求版本校验和取消机制。桌面弹窗、右侧栏和手机端各自持有独立搜索会话，互不取消或覆盖。发布版以标题搜索为主；当前工作树在标题无结果时提供受限的块级全文片段回退，但尚未宣称支持完整的原生筛选器或原生搜索页替代；后续计划见 [ROADMAP.md](./ROADMAP.md)。
...
| 行为 | 默认排序方式、最近打开记录、全屏模式（默认关，开启后打开即铺满窗口） |
