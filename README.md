...
[![Version](https://img.shields.io/badge/version-0.24.0-blue)](./plugin.json) [![License: MIT](https://img.shields.io/badge/license-MIT-green)](./LICENSE) [![SiYuan](https://img.shields.io/badge/SiYuan-%E6%80%9D%E6%BA%90%E7%AC%94%E8%AE%B0-ff5c67)](https://b3log.org/siyuan)
...
开发头目标版本为 `v0.24.0`（尚未正式发布；发布后本行更新为实际 tag）
...
**测试矩阵**：`pnpm test` 自动发现 `tests/` 与 `tests/host/` 下的 `*.test.cjs` 文件，当前共 6314 项测试（212 个测试文件），精确计数以命令输出为准；UI 冒烟测试单独执行：
...
pnpm test               # 自动运行全部单元、契约与宿主发版测试（当前 6314 项）
...
pnpm verify:release     # 发布候选本地总门禁（类型、双构建复现审计、6314 项测试、发布/质量/集成审计和三套 UI 冒烟）
```
