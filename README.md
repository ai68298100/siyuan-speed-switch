...
**测试矩阵**：`pnpm test` 自动发现 `tests/` 与 `tests/host/` 下的 `*.test.cjs` 文件，当前共 6238 项测试（206 个测试文件），精确计数以命令输出为准；UI 冒烟测试单独执行：
...
pnpm test               # 自动运行全部单元、契约与宿主发版测试（当前 6238 项）
...
pnpm verify:release     # 发布候选本地总门禁（类型、构建、6238 项测试和三套 UI 冒烟）
```
