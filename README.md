...
> 当前开发策略：开发头已通过类型检查、生产构建、6113 项自动测试、移动端与 Chromium UI 烟测；已接入时间、天气、空气质量、节假日日历、Bangumi 每日放送、DailyHotApi 热搜、NewsNow 资讯、Hacker News 榜单、ActivityWatch 使用时长、iCal 订阅日程、RSS/Atom 订阅、GitHub 贡献热力图、小驴打卡组件（需安装小驴打卡）与内核数据组件（置顶文档、收集箱、最近更新、数据健康、最近文档、数据库），组件商店新增“离线可用 / 本机服务 / 外部 API”来源筛选。同步期间组件面板保持稳定，结束或失败后合并刷新；Agent 仍保持既有只读审计与受控动作边界，不开放新的隐式写入；路径筛选真实宿主能力、窄侧栏、ActivityWatch 实机和 Android 真机验收继续作为兼容性补充。
...
**测试矩阵**：`pnpm test` 自动发现 `tests/` 与 `tests/host/` 下的 `*.test.cjs` 文件，当前共 6113 项测试（196 个测试文件），精确计数以命令输出为准；UI 冒烟测试单独执行：
