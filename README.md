...
> 当前开发策略：正式发布基线已通过类型检查、生产构建、1094 项自动测试、移动端与 Chromium UI 烟测；当前开发头新增 Agent 只读审计生命周期历史、回放与健康报告契约，仍不开放新的写入动作。收藏/置顶/分组容量边界已锁定（512/64/64），路径筛选真实宿主能力、窄侧栏和 Android 真机验收仍作为后续兼容性补充。
