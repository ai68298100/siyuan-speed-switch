...
- **兼容性**：对照思源 v3.8.4 内核源码完成兼容性调研，无破坏性变更；新组件全部基于
  只读端点，不新增任何写入或隐式数据流出。
- **工程质量**：自动测试 5872 → 6123（198 个测试文件）；11 个既有模块的边界加固；
...
**测试矩阵**：`pnpm test` 自动发现 `tests/` 与 `tests/host/` 下的 `*.test.cjs` 文件，当前共 6123 项测试（198 个测试文件），精确计数以命令输出为准；UI 冒烟测试单独执行：
...
pnpm test               # 自动运行全部单元、契约与宿主发版测试（当前 5779 项）
...
pnpm verify:release     # 发布候选本地总门禁（类型、构建、5779 项测试和三套 UI 冒烟）
```
