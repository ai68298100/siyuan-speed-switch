...
> 当前开发策略：继续以小步开发和自动化护栏为主，暂不强行整理为完整发布版本。类型检查、生产构建、575 项测试、移动端与 Chromium UI 烟测已通过；真实设备验收、安装升级卸载和正式发布等待维护者有时间审核，并且不会在用户确认前创建 Tag、Release 或推送。
...
**测试矩阵**：`pnpm test` 自动发现 `tests/` 与 `tests/host/` 下的 `*.test.cjs` 文件，当前共 575 项测试；UI 冒烟测试单独执行：
