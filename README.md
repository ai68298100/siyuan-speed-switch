...
> 当前开发策略：开发头已通过类型检查、生产构建、5884 项自动测试、移动端与 Chromium UI 烟测；已接入时间、天气、节假日日历、Bangumi 每日放送、DailyHotApi 热搜、NewsNow 资讯、ActivityWatch 使用时长、iCal 订阅日程与 GitHub 贡献热力图，组件商店新增“离线可用 / 本机服务 / 外部 API”来源筛选。同步期间组件面板保持稳定，结束或失败后合并刷新；Agent 仍保持既有只读审计与受控动作边界，不开放新的隐式写入；路径筛选真实宿主能力、窄侧栏、ActivityWatch 实机和 Android 真机验收继续作为兼容性补充。
...
- 发版门禁：5884 项自动测试（173 个测试文件）、TypeScript、生产构建、移动端 smoke、Chromium smoke 与 `verify:release` 全部通过；产物 dist/index.js 632957 bytes、package.zip 320945 bytes。
...
**测试矩阵**：`pnpm test` 自动发现 `tests/` 与 `tests/host/` 下的 `*.test.cjs` 文件，当前共 5884 项测试（173 个测试文件），精确计数以命令输出为准；UI 冒烟测试单独执行：
