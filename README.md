...
> `v0.16.20` 智能体受控写试点：新增 `update-task-status` 能力，AI 切换任务勾选状态前强制弹窗确认（拒绝/超时不执行）；命令翻转纯函数全分支测试覆盖。上一版 `v0.16.19` 组件面板活数据与手机适配：内置组件声明自动刷新时机（面板打开期间页签变化即时保鲜，500ms 防抖）；手机端组件画布强制单列堆叠（修复 12 列网格把组件压成窄条）；组件商店支持按名称/描述即时搜索。上一版 `v0.16.17` 协议 v2 与智能体结合：组件支持条目级命令、声明式配置表单、自动刷新时机与商店元数据；新增智能体能力 `home-widget-snapshot`（AI 可查询任意已注册组件的有界快照）。上一版 `v0.16.16` 组件面板生态化：面板更名为「组件面板」，组件商店常驻右上角；尺寸扩至 7 档（迷你 2×3 ～ 全幅 12×6）并以弹出菜单选择；新增思源原生组件——今日待办、标签、书签、本月日记、插件命令启动器（任何插件无需适配即可进面板）；发布第三方组件开发指南 docs/widget-protocol.md。上一版 `v0.16.15`：小组件改为固定尺寸型号（小 4×3 / 中 4×4 / 宽 8×3 / 大 8×5）+ 拖拽挪动自动归位 + 小组件商店画廊；切换器分组下小分组按标准卡片宽度并排流动，实时响应窗口变化；修复「分组·排序」浮层位置。上一版 `v0.16.14`：分组列表恢复与之前完全一致的标准卡片尺寸（不再拉伸），组头折叠保留；「分组 · 排序」改用与收藏/最近下拉同款的自制浮层（修复思源 Menu 被弹窗层级遮挡导致点击无下拉）；设置页桌面端改为独立 70% 视口自适应。上一版本的 `v0.16.13`：页签列表支持按笔记本（默认）/收藏/创建月份分组，与排序一体化设定，稀疏分组按内容并排流动；第二面板升级为可增删、挪动、调宽的小组件主页（内置最近/收藏/日记/文档集模块，支持第三方只读模块与失败跳转）；设置页桌面端同比例自适应；修复手机顶栏拥挤与设置页标签条错位。
...
> 当前开发策略：继续以小步开发和自动化护栏为主，暂不强行整理为完整发布版本。类型检查、生产构建、532 项测试、移动端与 Chromium UI 烟测已通过；真实设备验收、安装升级卸载和正式发布等待维护者有时间审核，并且不会在用户确认前创建 Tag、Release 或推送。
...
- 新增内置组件 **日历月视图**：本月日记一目了然（周一开头 7 列网格），日记日期高亮、点击直达，支持月份偏移回看历史；协议扩展 `viewType=calendar`。
...
- 内置组件达 27 个；智能体能力真正达到 10 项（5 只读 + 2 受控导航 + 3 受控写）；550 项自动测试。
...
- 组件面板新增两个内置组件：**闪卡待复习**（笔记本级到期闪卡，单本模式点击直达卡片文档、全部模式显示到期数比例条）与**随机回顾**（随机抽取久未翻动的旧文档，点「刷新全部」换一批）；内置组件达 20 个。
- 尺寸菜单新增**型号预览瓦片**：每档按 12 列比例绘制小矩形，并做视口钳制不再弹出屏外。
- 智能体新增两项能力：**批量打开文档** `open-documents`（一次最多 5 篇，弹窗列出全部标题后确认，拒绝或超时一篇不开）与**工作区上下文** `workspace-context`（设备端、活动文档、打开页签、文档集、快捷入口、今日日记状态一次只读汇总，日记只探测不创建）；智能体能力共 10 项。
- **真机验收修复四个潜伏缺陷**（本机思源 v3.8.2 实测）：① 内核文档搜索与 SQL 端点参数错误导致 SQL 类组件在真实宿主静默失败；② 组件尺寸菜单因注册时未继承内置尺寸而塌缩为单一档位；③ 标签/书签组件因 3.8.x 返回结构差异恒显 0；④ 全文内容搜索因空类型筛选从未命中（修复后实测由 0 到 12 篇命中）。
- 韧性增强：组件读取失败但存在上次好数据时继续展示旧快照并标注「缓存」，不再整块报错；错误态附带原因码便于自诊。
- 安全加固：全部内核请求改为字面量端点分发（配合既有白名单与同源守卫）；任务写入载荷统一钳制。
- 529 项自动测试；真实宿主验收清单见 docs/acceptance-v0.16.37.md。

### v0.16.36（2026-09-12）

- 搜索结果卡片显示舒适度：间距、内边距、标题字重与摘要行高统一放宽，长路径/摘要截断更从容。
- 组件商店新增分类页签：全部 / 内置 / 插件组件，第三方组件一眼可辨。
- 组件面板时间感知问候头：清晨/午后/夜晚自适应文案。
- 组件面板新增**刷新全部**按钮：一键绕过缓存与失败退避，强制重读所有组件数据。

### v0.16.35（2026-09-12）

- 智能体新能力 `get-document-outline`（只读）：返回指定文档的标题大纲（标题文本、层级、标题块 ID），可与「打开文档」配合按章节定位；结果有界（最多 48 条，深度钳制 0–8）。
- 安全加固：内核请求统一入口增加**同源守卫**——仅允许以 `/` 开头的相对路径且必须命中端点白名单，显式拒绝绝对 URL 与外域 host。
- 智能体能力共 8 项（4 只读 + 1 受控导航 + 3 受控写）；522 项自动测试。

### v0.16.34（2026-09-12）

- 内置组件库扩充（参考社区仪表盘插件的统计口径）：新增 **笔记统计**（全库文档数 + 字数估算 + 本周新建/改动，聚合 SQL）、**年度进度**（纯前端计算已过/剩余天数与百分比，带进度条）、**近期编辑**（全库最近修改的 10 篇文档，点击直达）。
- 组件渲染增强：概览进度条（`stat.progress`）与条目比例条（`item.count`，按最大值归一化）上线，用于年度进度与标签分布等场景。
- 内置组件总数达到 13 个；组件目录与商店自动收纳新组件。

### v0.16.33（2026-09-12）

- 组件面板视觉深化：概览进度条、条目比例条（标签按出现次数归一化）、卡片强调色微底色（3% color-mix）。
- 内置组件扩充：新增 **本周新建**（本周创建的文档）、**笔记本概览**（各笔记本文档数比例条）。
...
- 组件商店分区：分为「可用组件」与「需安装插件后可用」两大区。可用区展示内置组件与已注册的第三方插件组件；待安装区展示组件目录（`src/widget-catalog.js`）中已登记、但来源插件未就位的组件，卡片标注需安装的插件名。
- 安装状态自动识别：打开组件商店即读取思源已加载插件列表完成识别（同步遍历、零额外开销），并在 400ms 后异步复扫一次以覆盖插件启停变化；识别结果驱动两个分区的展示与迁移。
- 面板健壮性：来源插件被卸载后，面板中遗留的组件实例自动隐藏（不再显示错误卡片），重新安装后自动恢复。

### v0.16.29（2026-09-12）

- 性能优化：设置对象记忆化——`getSettings()` 的规范化结果按需缓存、写入时失效，渲染/绑定等高频路径零重复开销。
- 侧栏刷新合并：文档批量打开/关闭时的 `loaded-protyle`/`destroy-protyle` 事件连发合并为一次侧栏重建（150ms 尾沿），不再逐事件全量重建 DOM。
- 今日待办全库扫描默认加近 30 天时间窗（7–365 天可配），配合思源内核 SQL 的结果上限，避免大库查询退化；相关实现已对照思源最新内核源码（`kernel/api/block.go`、`kernel/model/block.go`、`kernel/api/search.go`）与前端事件总线（`app/src/plugin/EventBusCore.ts`）确认。

### v0.16.28（2026-09-12）

- 组件面板独立尺寸设置：设置页新增「组件面板」标签页，尺寸模式支持跟随第一面板（默认）/自适应屏幕（90%）/固定尺寸/全屏铺满；固定尺寸模式可单独设置面板宽高（480–1920 × 360–1280），与第一面板互不影响。
- 组件面板底端快捷入口栏：与第一面板共享同一份快捷入口配置（图标展示、底端常驻），配置改动双向同步；面板结构调整为顶栏/滚动区/底栏三段，长列表不再把入口栏顶走。
- 全屏模式下组件面板网格限宽 1600px 居中，超宽屏不再出现过宽列表。

### v0.16.27（2026-09-12）

- 今日待办组件新增「显示已完成」配置（默认关）：开启后已完成任务以划线样式展示，可点击勾选框重新打开；配合任务勾选写入形成完整闭环。
- 全库扫描模式（扫描全部文档=是）默认仅检索近 30 天有更新的任务，时间范围 7–365 天可配，避免大库下查询退化。
- ROADMAP AI 能力专项同步至当前七项能力现状（三项只读、一项受控导航、三项受控写/规划中）。

### v0.16.26（2026-09-12）

- 手机端组件面板入口：底栏快捷入口新增「组件面板」常驻按钮（与日记/搜索/设置并列），手机用户不再需要绕道"更多"菜单即可直达组件面板。

### v0.16.25（2026-09-11）

- 受控写第二试点 `append-to-journal`：AI 向今日日记末尾追加一条内容（日记不存在时自动创建，日记笔记本取自插件设置），执行前强制弹窗确认（拒绝/超时不执行）。
- 追加内容经清洗（压平换行、合并空白、限长 512），仅在日记末尾追加列表项，不改写已有内容。

### v0.16.24（2026-09-11）

- refreshOn 刷新细化：事件触发时只刷新订阅了该事件的组件，不再整面板重读。
- 今日待办全库扫描加时间窗守卫：默认仅扫描近 30 天有更新的任务（天数 7–365 可配），防止大库下 LIKE 查询退化。
- 智能体能力表补全 `update-task-status` 与 `create-document` 文档。

### v0.16.23（2026-09-11）

- 受控写扩展：智能体新能力 `create-document`——在指定笔记本下新建文档（可带初始 Markdown 内容），执行前强制弹窗确认（拒绝/超时不执行）；`notebook` 参数支持笔记本 ID 或名称。
- 今日待办组件支持按笔记本过滤：配置表单新增动态笔记本下拉（协议新增 `notebook` 配置字段类型，选项由面板按思源笔记本列表动态填充）。

### v0.16.22（2026-09-11）

- 智能体发现模式：`home-widget-snapshot` 省略 `moduleId` 时返回面板上全部可查询组件的清单（含名称、描述与尺寸型号），AI 可先发现再查询。
- 组件配置直达：带 `configSchema` 的组件（如指定文档、今日待办）在头部常驻「配置」按钮，无需进入编辑布局即可调整配置并即时刷新。
- 协议文档同步：[docs/widget-protocol.md](docs/widget-protocol.md) 补充发现模式说明。

### v0.16.21（2026-09-11）

- 面板内直接勾选待办：今日待办组件条目自带勾选框，点击即在思源中将任务标记完成/重开（写入思源块，不跳转），完成后条目划线置灰并即时刷新；写失败有消息反馈。
- 协议与渲染链路：快照条目支持可选 `done` 布尔标记，勾选交互由面板按 `onToggleItem` 回调渲染，第三方组件同样可用。

### v0.16.20（2026-09-11）

- 智能体受控写试点 `update-task-status`：AI 可请求切换任务勾选状态，执行前强制弹窗确认（拒绝/超时不执行），effects 如实声明 localWrite；勾选翻转纯函数全分支覆盖。

### v0.16.19（2026-09-11）

- 组件面板“活数据”：内置组件声明自动刷新时机（最近打开/收藏/今日待办/文档集订阅页签变化事件），面板打开期间数据自动保鲜（500ms 防抖）；第三方组件可通过协议 v2 的 `refreshOn` 声明同样能力。
- 修复手机端组件画布：12 列网格在窄屏会把组件压成约 120px 宽的窄条且旧的单列降级选择器匹配不到，现按设备注入单列类名强制全宽堆叠。
- 组件商店新增即时搜索框：按组件名称与描述过滤，分区随结果自动隐藏。
...
- 「第二面板」更名为**组件面板**；组件商店按钮常驻右上角，不再需要先进编辑布局。
- 组件尺寸扩至 7 档：迷你 2×3 / 小 4×3 / 方 4×4 / 高 4×6 / 宽 8×3 / 大 8×5 / 全幅 12×6；每个模块声明支持的型号子集，编辑时以弹出菜单选择全部档位（替代单一循环按钮）；旧宽度档自动就近迁移。
- 新增 5 个思源原生组件：**今日待办**（SQL 扫描打开文档的未完成任务块，点击跳块）、**标签** / **书签**（getTag / getBookmark，点击打开对应侧栏面板）、**本月日记**（按日记标题前缀聚合当月日记，内置“打开今日日记”快捷项）、**插件命令启动器**（枚举其他插件的命令一键触发——任何插件无需适配即可进面板）。
- 发布第三方组件开发指南 [`docs/widget-protocol.md`](docs/widget-protocol.md)：注册签名、快照数据契约、尺寸型号、跳转回调与完整示例，供日历/日记/待办/番茄钟等插件作者接入组件商店。
- 组件面板底部新增常驻提示：面板持续完善中，并附《组件接入指南》链接邀请插件开发者上架组件。
- 修复桌面侧边栏顶栏重复的“收藏/排序”裸标签，排序控件升级为与弹窗一致的分组·排序一体化浮层（侧栏从此可改分组方式）；修复手机端切换器顶栏换行规则被旧样式压制导致的单行挤压（搜索框消失、按钮重叠），恢复两行结构并提升芯片触控高度。

### v0.16.15（2026-09-10）

- 第二面板 iPad 式小组件画布：12 列固定网格 + 40px 行单元 + dense 自动归位；每个模块预设固定尺寸型号，模块声明自己支持的型号集合，编辑时在型号间切换；旧宽度档自动就近迁移。
- 编辑态支持桌面拖拽挪动（松手后自动紧凑归位，无重叠无空洞），手机端保留上移/下移按钮。
- 「添加模块」升级为小组件商店画廊：内置/插件分区，卡片含图标、说明与型号瓦片，点瓦片即添加；已添加模块的点瓦片可直接改型号。
- 模块内容随型号适配密度（小号紧凑排版，列表在格子内滚动）。
- 切换器分组列表：小分组（1~2 卡）按标准卡片宽度并排流动换行，大分组占满整行；卡片尺寸与不分组逐像素一致；容器宽度变化经 ResizeObserver 实时重排（rAF 去抖，不重建卡片）；方向键在流式布局下按屏幕坐标就近移动。
- 修复「分组 · 排序」浮层出现在弹窗左下角的问题：浮层挂载到 body 并以高 z-index 兜底，窗口缩放实时跟随按钮位置。

### v0.16.14（2026-09-10）

- 分组列表回归标准网格：分组只改变“怎么分组”，卡片尺寸与排列与之前完全一致；组头（图标 + 名称 + 计数）可折叠保留。
- 修复点击「分组 · 排序」无下拉的问题：弃用思源 Menu（疑似被插件弹窗层级遮挡），改为与收藏/最近下拉同款的自制浮层（fixed 定位、外点关闭、Esc 关闭）。
- 设置页桌面端改为独立的 70% 视口自适应比例，不再与第一面板的自适应比例设置联动。
- 第二面板维持两卡片列表模式（默认最近打开 + 收藏），编辑布局能力保持可用。

### v0.16.13（2026-09-10）

- 页签列表分类分组：新增「分组 · 排序」一体化菜单（桌面）与排序面板分组区（手机），分组方式支持按笔记本（默认）/按收藏/按创建月份/不分组，组内沿用六种排序；侧栏与手机列表共享同一分组语义。
- 稀疏分组布局：分组块按内容卡片数自适应宽度并横向流动换行，组头显示图标、名称与计数，点击可折叠。
- 第二面板小组件化：面板改由 home 运行时驱动，内置最近打开、收藏、今日日记、文档集、指定文档等只读模块；支持添加/移除/排序/宽度与折叠，实例按端持久化；第三方插件可通过 registerHomeModule 注册只读模块。
- 设置页桌面端跟随面板尺寸模式自适应打开（手机端维持视口收缩）。
- 修复手机端切换器顶栏拥挤与设置页标签条错位；文档集设置的长说明移至保存/导出/导入按钮下方；发布包体预算重校准。

### v0.16.12（2026-09-10）

- 第二面板新增顶栏独立入口与默认快捷键 `Alt+Shift+P`；命令同时注册全局回调，在思源 **设置 → 快捷键** 为其绑定"全局"后，可在思源未获得焦点时用系统级热键唤起（触发时思源同时回到前台）。
- 桌面端第一面板与第二面板尺寸默认按屏幕比例自适应（90%，可在设置调整 50–100）；外观设置新增"面板尺寸模式"：自适应屏幕比例 / 固定尺寸 / 全屏铺满，替代原先行为页的独立全屏开关；弹窗内全屏按钮仍可临时切换。
- 快捷入口默认值一次性迁移：升级到本版后，旧版自动写入快捷栏的条目（含第三方插件入口）重置为默认的搜索、日记、设置三项；此后的人工自定义永久保留。
- 修复手机端顶栏切换器入口与宿主尾部按钮（关闭/更多）重叠的问题：入口按钮固定插入宿主尾部按钮之前，旧版本升级残留的错位按钮也会自动归位。
- 发布包体优化：esbuild 目标提升至 `es2020`（不再为现代 WebView 降级 async/await）并剥离压缩产物中的许可证注释，`dist/index.js` 回落到移动端性能预算内（约 229KiB，LICENSE 仍以完整文本保留在产物头部）。
- 发布归档审计测试改用零依赖 zip 解析（`tests/host/lib/zip.cjs`），不再调用宿主 `tar`，在 Git Bash / GNU tar 环境下也能稳定复现。
...
- 适配思源 3.8.3 的 `MobileTabs` 元数据，移动页签保留根文档、笔记本和路径信息。
- 规范化快捷入口候选项的插件元数据和多行文本，未知平台能力保持可见但不会被错误标记为手机可用。
- 标题搜索无结果时增加受限全文块搜索回退，并按根文档聚合命中片段；该回退受容量限制，不替代思源原生搜索页。
...
- 新安装默认仅保留搜索、日记和设置三个可配置入口，并固定提供一个第二面板入口；第三方插件动作只进入“添加入口”候选，不再自动占用快捷栏。旧用户已有配置继续保留。
...
- **质量门**：tsc 0 错、build 成功（当前约 237 KiB package.zip，硬上限 300 KiB）、入口 bundle 不超过 230 KiB、零 console / debugger / alert / `@ts-ignore` 残留、`as any` 仅 1 处（在注释里，非代码）。
...
**测试矩阵**：`pnpm test` 自动发现 `tests/` 与 `tests/host/` 下的 78 个 `*.test.cjs` 文件，当前共 532 项测试；UI 冒烟测试单独执行：
