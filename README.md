...
> 当前开发策略：继续以小步开发和自动化护栏为主，暂不强行整理为完整发布版本。类型检查、生产构建、580 项测试、移动端与 Chromium UI 烟测已通过；真实设备验收、安装升级卸载和正式发布等待维护者有时间审核，并且不会在用户确认前创建 Tag、Release 或推送。
...
- 内置组件达 27 个；智能体能力真正达到 10 项（5 只读 + 2 受控导航 + 3 受控写）；674 项自动测试。
...
早期 v0.6.1–v0.3.0 版本主要完成收藏分组、侧栏、搜索和主题适配；细节已沉淀在 Git 历史中，当前归档仅保留可追溯的版本范围摘要。
...
**测试矩阵**：`pnpm test` 自动发现 `tests/` 与 `tests/host/` 下的 `*.test.cjs` 文件，当前共 674 项测试；UI 冒烟测试单独执行：
